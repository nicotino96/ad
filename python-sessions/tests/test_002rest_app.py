...
class IdeAPIHealthEndpointTestCase(unittest.TestCase):

    def test_health_endpoint_has_correct_bool(self):
        def test_body():
            self.__assert_endpoint("v1/health", {"is_living": True})

        # Tiny check - Was views.py removed?
        self.assertFalse(os.path.exists(context.IDEAPI_ROOT + "idearest25app/views.py"))
