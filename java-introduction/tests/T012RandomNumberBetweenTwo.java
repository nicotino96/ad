...
public class T012RandomNumberBetweenTwo {

    @Test
    public void testMethodReturnValue() {
        try {
            Class cls = Class.forName("SimpleMathDemo");
            Method method = cls.getDeclaredMethod("randomNumberBetween", int.class, int.class);
            Object instance = cls.getDeclaredConstructor().newInstance();
            for (int i = 0; i < 50; i++) {
                // Test 50 times with random values
                int num1 = (int) Math.floor(Math.random()*100) - 50;
                int num2 = (int) Math.floor(Math.random()*100) + 100;
