...
class WallAPIProjectAndAppExistTestCase(unittest.TestCase):
    ...
    def test_django_project_exists(self):
        ...
        self.assertTrue(os.path.exists(context.WALL_API_ROOT))
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def __test_with_server_alive(self, block):
        def run_server():
            proc = subprocess.Popen("python manage.py runserver", creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, cwd=context.WALL_API_ROOT)
            time.sleep(4)
