...
class SimpleAPIExamplePostEndpointTestCase(unittest.TestCase):

    def test_correct_get_error_response(self):
        def test_body():
            self.__assert_endpoint("resource/8", {"error": "HTTP method not supported"}, None)
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_correct_post_response(self):
        def test_body():
            self.__assert_endpoint("resource/14", {"message": "You have sent a POST to the resource 14"}, 200, 'POST')
        # Run server and run test
        self.__test_with_server_alive(test_body)
