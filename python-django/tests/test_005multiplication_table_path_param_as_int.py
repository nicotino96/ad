...
class SimpleAPIMultiplicationPathParamAsIntEndpointTestCase(unittest.TestCase):

    def test_rand_number1to10_produce_correct_http_response(self):
        def test_body():
            r = random.randint(1, 11)
            self.__assert_endpoint("v2/multiplications/" + str(r), self.__table(r))
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_rand_number_gt100_produce_correct_http_response(self):
        def test_body():
