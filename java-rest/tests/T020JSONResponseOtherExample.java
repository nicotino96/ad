...
public class T020JSONResponseOtherExample {
    private Method stopServer = null;
    private Object currentServerInstance = null;

    @Test
    public void testExampleEndpointCharacterDescription() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8125/example4");
            assert(result != null);
            assertEquals("Lisa Simpson es la saxofonista que adora el Jazz y a los animales", result.getString("characterDescription"));
    ...
    @Test
    public void testExampleEndpointCharacterPopularity() {
        try {
            Class cls = Class.forName("SimpleREST");
            this.currentServerInstance = cls.getDeclaredConstructor().newInstance();
            this.stopServer = cls.getDeclaredMethod("stopServer");
            ...
            JSONObject result = getUrl("http://localhost:8125/example4");
            assert(result != null);
            assertEquals(3, result.getInt("characterPopularity"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
