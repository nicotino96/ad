...
public class T022ValuableCompanyv2 {
    ...
    public void testEndpointInnerJSON() {
        try {
            ...
            Class cls2 = Class.forName("JSONGetCompany2");
            ...
            JSONObject result = getUrl("http://localhost:8125/otherCompany");
            assert(result != null);
            JSONObject profitJson = result.getJSONObject("grossProfit");
            assert(profitJson != null);
            assertEquals(2016, profitJson.getInt("year"));
            assertEquals(5012308L, profitJson.getLong("amount"));
            assertEquals("EUR", profitJson.getString("currencyCodeIso4217"));
    ...
    public void testEndpointOuterJSON() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8125/otherCompany");
            assert(result != null);
            assertEquals("HBA", result.getString("name"));
            assertEquals("Alice Doe", result.getString("owner"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
