...
public class T021ValuableCompany {
    ...
    public void testEndpoint() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8125/company");
            assert(result != null);
            JSONObject profitJson = result.getJSONObject("grossProfit");
            assert(profitJson != null);
            assertEquals("Amazon", result.getString("name"));
            assertEquals("Jeff Bezos", result.getString("owner"));
            assertEquals(2021, profitJson.getInt("year"));
            assertEquals(54577000000L, profitJson.getLong("amount"));
            assertEquals("USD", profitJson.getString("currencyCodeIso4217"));
