...
public class T016DeleteAFilm {
    ...
    public void testCorrectDeletion() {
        ...
        try {
            ...
            Method method = cls.getDeclaredMethod("wipeFilm", int.class);
            boolean result = (boolean) method.invoke(instance, newMovieId);
            assertTrue(result);
        ...
        try {
            ...
            ResultSet result = statement.executeQuery("SELECT COUNT(*) FROM TMovies WHERE id = " + newMovieId);
            while(result.next()) {
                newFilmCount = result.getInt(1);
            }
            conn.close();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        assertEquals(0, newFilmCount);
    }
    ...
    public void testReturnsFalse() {
        ...
        try {
            ...
            Object instance = cls.getDeclaredConstructor().newInstance();
            Method method = cls.getDeclaredMethod("wipeFilm", int.class);
            boolean result = (boolean) method.invoke(instance, invalidRandomId);
            assertFalse(result);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
